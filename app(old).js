...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: "pandiaraj984@gmail.com",
    pass: "wpnh aiyt xrwz teyp",
  },
})
const sendEmail = async(mailto, content) => {
  const mailOptions = {
    from : {
      ...
      address: "mgowtham7577@gmail.com"
    },
    to: mailto,
...
app.get('/testmail', async (req, res)=>{
    try{
        const mailDeliveryStatus = await sendEmail('subhinkrishna.suresh@gmail.com',141414)
...
app.post('/update-user', isAdmin, async(req, res)=>{
    try{
        ...
        if(formData.password&&formData.password!==''){
            const userUpdation = await userModel.updateOne({id: formData.id}, { $set: {
                fullname: formData.fullname,
                email: formData.email,
                contact: formData.contact,
                username: formData.username,
...
const fetchDevices = () => {
    return fetch('https://api.thingspeak.com/channels/2774098/feeds.json?api_key=GQY07L8LPM9DDHJM&results')
        .then(res => res.json())
        .catch(err => {
...
const checkLockStatus = async (containerID) => {
    try {
        ...
        if (data.channel.id === Number(containerID)) {
            ...
            return lastFeed.field8 === '1.00000' ? 'locked' : 'unlocked';
        }
                ...
                if(validatedDrugName===data){
                    ...
                    if(enteredQuantity>availableQuantity){
                        ...
                        if(drugUpdateStatus){
                            ...
                            // const mailto = ['subhinkrishna.sk@gmail.com',Manufacturer.email,Receiver.email]
...
//         if (!drugData || !Array.isArray(drugData.drugNames) || !Array.isArray(drugData.drugQuantities)) {
//             return res.send({ success: false, message: "Invalid drug data or missing drug details!" });
//         }

//         const drugNames = drugData.drugNames;  // Assuming drugNames is an array
//         const drugQuantities = drugData.drugQuantities; // Assuming drugQuantities is an array of strings
...
//         const availableQuantity = parseFloat(drugQuantities[drugIndex]);

//         if (availableQuantity <= 0) {
//             return res.send({ success: false, message: `No available quantity for ${formData.drugName}!` });
//         }

//         if (availableQuantity < parseFloat(formData.quantity)) {
...
//         const updatedQuantity = availableQuantity - parseFloat(formData.quantity);

//         // Update drug data with the new quantity
//         await contract.updateDrugQuantity(formData.urn, validatedDrugName, updatedQuantity.toString());
...
app.get('/getAllContainers', isAuth, async (req, res) => {
    try {
        ...
        const groupedById = formattedContainers.reduce((acc, container) => {
...
const checkStatus = async () => {
    try {
        ...
        // Check if the last feed value is '1.00000'
        if (lastFeed.field8 !== '1.00000') {
            // If not, call your desired function
...
const sendNotificationEmail = (lat, lon, containerId) => {
    const mailOptions = {
      ...
      to: ['spiderspoider@gmail.com', 'josewin067@gmail.com'], // Replace with NCB admins email
